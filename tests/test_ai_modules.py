...
TEST_IMAGES_SUBDIR = os.path.join(
    TEST_DATA_DIR, "images"
)  # Subdirectory for test images
...
TEST_PARIETAL_MODEL_PATH = os.path.join(
    TEST_DATA_DIR, "test_parietal_model.json"
)  # Added for ParietalLobeAI
...
class TestDataLoading(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.vision_content = {
            "image_paths": [os.path.join(TEST_IMAGES_SUBDIR, "img1.png")]
        }
        ...
        cls.pairs_content = [
            {"image_path": "img1.png", "text_description": "text1", "visual_label": 0},
...
class TestTemporalLobeAI(unittest.TestCase):
    ...
    def test_forward_prop_text_output_shapes(self):
        ...
        _, text_hidden_output, text_embedding_scores = self.ai._forward_prop_text(
            input_vec_1d
        )
        self.assertEqual(text_hidden_output.shape, (self.ai.text_hidden_size,))
        self.assertEqual(
            text_embedding_scores.shape, (self.ai.output_size,)
        )  # output_size is embedding_size

    def test_forward_prop_visual_assoc_output_shapes(self):
        test_embedding_vec = np.random.rand(
            self.ai.output_size
        )  # output_size is embedding_size
        # Expected return: visual_input_vec (embedding), visual_hidden_output, visual_label_scores
        _, visual_hidden_output, visual_label_scores = (
            self.ai._forward_prop_visual_assoc(test_embedding_vec)
        )
        self.assertEqual(
            visual_hidden_output.shape, (self.ai.visual_assoc_hidden_size,)
        )
    ...
    def test_learn_updates_all_weights_and_biases(self):
        initial_w = [
            p.copy()
            for p in [
                self.ai.weights_text_input_hidden,
                self.ai.bias_text_hidden,
                self.ai.weights_text_hidden_embedding,
                self.ai.bias_text_embedding,
                self.ai.weights_embed_to_visual_hidden,
                self.ai.bias_visual_assoc_hidden,
                self.ai.weights_visual_hidden_to_label,
                self.ai.bias_visual_assoc_label,
            ]
        ]
    ...
    def test_consolidate_updates_all_weights_and_biases(self):
        ...
        for i_p, f_p in zip(initial_params, final_params):
            self.assertFalse(
                np.array_equal(i_p, f_p),
                "A weight or bias attribute was not updated during consolidate.",
            )

    def test_model_save_load_new_temporal_architecture(self):
        # Set known, distinct values for a selection of weights and biases
        self.ai.weights_text_input_hidden = np.full(
            (self.ai.input_size, self.ai.text_hidden_size), 0.11
        )
        self.ai.bias_text_embedding = np.full(
            (1, self.ai.output_size), 0.22
        )  # output_size is embedding_size
        self.ai.weights_visual_hidden_to_label = np.full(
            (self.ai.visual_assoc_hidden_size, self.ai.visual_output_size), 0.33
        )
        self.ai.bias_visual_assoc_hidden = np.full(
            (1, self.ai.visual_assoc_hidden_size), 0.44
        )

        # Save and load the model
        self.ai.save_model()
        loaded_ai = TemporalLobeAI(
            model_path=TEST_TEMPORAL_MODEL_PATH, memory_path=TEST_TEMPORAL_MEMORY_PATH
        )

        # Assert that the loaded AI instance has the correct values for the set weights/biases
        np.testing.assert_array_almost_equal(
            loaded_ai.weights_text_input_hidden, self.ai.weights_text_input_hidden
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.bias_text_embedding, self.ai.bias_text_embedding
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.weights_visual_hidden_to_label,
            self.ai.weights_visual_hidden_to_label,
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.bias_visual_assoc_hidden, self.ai.bias_visual_assoc_hidden
        )
        ...
        self.assertEqual(
            loaded_ai.visual_assoc_hidden_size, self.ai.visual_assoc_hidden_size
        )
...
class TestLimbicSystemAI(unittest.TestCase):
    ...
    def test_forward_propagate_output_shapes_limbic(self):
        ...
        input_vec_1d, hidden_layer_output_1d, output_scores_1d = (
            self.ai._forward_propagate(self.sample_temporal_output)
        )
    ...
    def test_learn_updates_and_memory_limbic(self):
        ...
        self.ai.learn(
            self.sample_temporal_output, self.sample_true_emotion, self.sample_reward
        )

        self.assertFalse(
            np.array_equal(initial_params["w_ih"], self.ai.weights_input_hidden),
            "weights_input_hidden did not change after learn",
        )
        self.assertFalse(
            np.array_equal(initial_params["b_h"], self.ai.bias_hidden),
            "bias_hidden did not change after learn",
        )
        self.assertFalse(
            np.array_equal(initial_params["w_ho"], self.ai.weights_hidden_output),
            "weights_hidden_output did not change after learn",
        )
        self.assertFalse(
            np.array_equal(initial_params["b_o"], self.ai.bias_output),
            "bias_output did not change after learn",
        )
    ...
    def test_consolidate_updates_weights_limbic(self):
        ...
        self.assertFalse(
            np.array_equal(initial_params["w_ih"], self.ai.weights_input_hidden),
            "weights_input_hidden did not change after consolidate",
        )
        self.assertFalse(
            np.array_equal(initial_params["b_h"], self.ai.bias_hidden),
            "bias_hidden did not change after consolidate",
        )
        self.assertFalse(
            np.array_equal(initial_params["w_ho"], self.ai.weights_hidden_output),
            "weights_hidden_output did not change after consolidate",
        )
        self.assertFalse(
            np.array_equal(initial_params["b_o"], self.ai.bias_output),
            "bias_output did not change after consolidate",
        )

    def test_model_save_load_new_limbic_architecture(self):
        # Set known values for specified attributes
        self.ai.weights_input_hidden = np.full(
            (self.ai.input_size, self.ai.hidden_size), 0.77
        )
        ...
        # Assert that the loaded AI instance has the correct values for the set weights/biases
        np.testing.assert_array_almost_equal(
            loaded_ai.weights_input_hidden, self.ai.weights_input_hidden
        )
        ...
        np.testing.assert_array_almost_equal(
            loaded_ai.bias_hidden, expected_bias_hidden
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.weights_hidden_output, expected_weights_hidden_output
        )
...
class TestOccipitalLobeAI(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        Image.new("L", cls.img_size, 0).save(cls.black_img_path)
    ...
    def tearDownClass(cls):
        ...
        if os.path.exists(TEST_OCCIPITAL_MODEL_PATH):
            os.remove(TEST_OCCIPITAL_MODEL_PATH)  # Can be removed if tearDown is robust
    ...
    def test_forward_propagate_output_shapes(self):
        # Unpack with clearer variable names
        input_vec_1d, hidden_layer_output_1d, output_scores_1d = (
            self.ai._forward_propagate(self.black_img_path)
        )
        self.assertEqual(
            input_vec_1d.shape, (self.ai.input_size,), "Input vector shape mismatch"
        )
        self.assertEqual(
            hidden_layer_output_1d.shape,
            (self.ai.hidden_size,),
            "Hidden layer output shape mismatch",
        )
        self.assertEqual(
            output_scores_1d.shape,
            (self.ai.output_size,),
            "Output scores shape mismatch",
        )
    ...
    def test_learn_updates_and_memory(self):
        ...
        self.ai.learn(
            self.black_img_path, 0
        )  # Corrected target label to 0 for consistency
        ...
        self.assertIn((self.black_img_path, 0), self.ai.memory)

    def test_consolidate_updates_weights(self):
        self.ai.learn(self.black_img_path, 1)  # Ensure memory has data
        initial_params = {
            "w_ih": self.ai.weights_input_hidden.copy(),
            "b_h": self.ai.bias_hidden.copy(),
            "w_ho": self.ai.weights_hidden_output.copy(),
    ...
    def test_model_save_load_new_architecture(self):
        # Set known values for specified attributes
        self.ai.weights_input_hidden = np.full(
            (self.ai.input_size, self.ai.hidden_size), 0.55
        )
...
class TestFrontalLobeAI(unittest.TestCase):
    ...
    def test_process_task_epsilon_greedy_selection(self):
        ...
        self.assertTrue(
            len(set(action_counts_explore.values())) > 1 or self.ai.output_size == 1
        )
        ...
        self.assertEqual(
            ...
            1,
            "Should pick action with highest Q-value when epsilon is 0",
        )
        ...
        self.assertAlmostEqual(
            self.ai.exploration_rate_epsilon,
            max(self.ai.min_epsilon, initial_epsilon - self.ai.epsilon_decay_rate),
            "Epsilon decay is not working as expected",
        )
    ...
    def test_learn_q_update_and_memory(self):
        ...
        next_state = np.random.rand(
            self.ai.input_size
        ).tolist()  # Uses current self.ai.input_size
        done = False
        ...
        expected_weights_for_action_after_learn = (
            initial_weights_for_action
            + self.ai.learning_rate_q * error * state_vector_1d
        )

        self.ai.learn(state, action_taken, reward, next_state, done)

        np.testing.assert_array_almost_equal(
            self.ai.weights[:, action_taken],
            expected_weights_for_action_after_learn,
            decimal=5,
        ...
        for i in range(self.ai.output_size):
            if i != action_taken:
                np.testing.assert_array_equal(
                    self.ai.weights[:, i],
                    initial_weights_all_actions[:, i],
                    err_msg=f"Weights for action {i} (not taken) should not change.",
                )

        self.assertIn(
            (list(state), action_taken, reward, list(next_state), done), self.ai.memory
        )
        ...
        self.assertEqual(
            len(self.ai.memory),
            self.ai.max_memory_size,
            "Memory size limit not enforced.",
        )
    ...
    def test_consolidate_experience_replay(self):
        ...
        self.assertTrue(
            len(self.ai.memory) > 0, "Memory should be populated before consolidation."
        )
        initial_weights = self.ai.weights.copy()
        self.ai.consolidate()  # This should perform experience replay and update weights
        self.assertFalse(
            np.array_equal(initial_weights, self.ai.weights),
    ...
    def test_model_save_load_q_learning_params(self):
        ...
        self.assertAlmostEqual(
            loaded_ai.exploration_rate_epsilon,
            0.678,
            "Exploration rate not loaded correctly.",
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.weights,
            known_weights_current_format,
            err_msg="Weights not loaded correctly from new format.",
        )
        self.assertEqual(
            loaded_ai.weights.shape,
            current_weights_shape,
            "Loaded weights shape mismatch (new format).",
        )
        ...
        old_weights_data = (
            np.random.rand(old_input_size, self.ai.output_size) * 0.3
        ).tolist()
        ...
        self.assertEqual(
            loaded_ai_old_format.exploration_rate_epsilon,
            1.0,
        ...
        self.assertEqual(
            loaded_ai_old_format.weights.shape,
            (self.ai.input_size, self.ai.output_size),
        ...
        self.assertTrue(
            np.any(loaded_ai_old_format.weights),
            "Re-initialized weights should not be all zero (assuming default init).",
        )
...
class TestCerebellumAI(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_true_command = (
            np.random.rand(self.ai.output_size) * 2 - 1
        ).tolist()
    ...
    def test_prepare_input_vector(self):
        ...
        np.testing.assert_array_almost_equal(
            vec_long, np.array(long_data[: self.ai.input_size]), decimal=5
        )
    ...
    def test_prepare_target_command_vector(self):
        ...
        short_data = self.sample_true_command[:-1]
        vec_short = self.ai._prepare_target_command_vector(short_data)
        ...
        np.testing.assert_array_almost_equal(
            vec_long, np.array(long_data[: self.ai.output_size]), decimal=5
        )

    def test_forward_propagate_output_shapes_and_ranges(self):
        input_vec_1d, hidden_layer_output, final_commands_output = (
            self.ai._forward_propagate(self.sample_sensor_data)
        )
        self.assertEqual(
            input_vec_1d.shape, (self.ai.input_size,), "Input vector shape mismatch."
        )
        ...
        self.assertEqual(
            final_commands_output.shape,
            (self.ai.output_size,),
            "Final commands output shape mismatch.",
        )
        self.assertTrue(
            np.all(hidden_layer_output >= -1) and np.all(hidden_layer_output <= 1),
            "Hidden output out of Tanh range.",
        )
        self.assertTrue(
            np.all(final_commands_output >= -1) and np.all(final_commands_output <= 1),
            "Final commands out of Tanh range.",
        )
    ...
    def test_learn_updates_and_memory(self):
        ...
        self.assertFalse(
            np.array_equal(initial_w_ih, self.ai.weights_input_hidden),
            "weights_input_hidden should change after learn",
        )
        self.assertFalse(
            np.array_equal(initial_b_h, self.ai.bias_hidden),
            "bias_hidden should change after learn",
        )
        self.assertFalse(
            np.array_equal(initial_w_ho, self.ai.weights_hidden_output),
            "weights_hidden_output should change after learn",
        )
        self.assertFalse(
            np.array_equal(initial_b_o, self.ai.bias_output),
            "bias_output should change after learn",
        )

        expected_mem_sensor = (
            self.sample_sensor_data
            if isinstance(self.sample_sensor_data, list)
            else self.sample_sensor_data.tolist()
        )
        expected_mem_command = (
            self.sample_true_command
            if isinstance(self.sample_true_command, list)
            else self.sample_true_command.tolist()
        )
        ...
        for i in range(105):
            self.ai.learn(
                self.sample_sensor_data,
                (np.array(self.sample_true_command) * (i / 105.0)).tolist(),
            )
    ...
    def test_consolidate_updates_weights(self):
        ...
        self.assertFalse(
            np.array_equal(w_ih_before, self.ai.weights_input_hidden),
            "weights_input_hidden should change after consolidate",
        )
        self.assertFalse(
            np.array_equal(b_h_before, self.ai.bias_hidden),
            "bias_hidden should change after consolidate",
        )
        self.assertFalse(
            np.array_equal(w_ho_before, self.ai.weights_hidden_output),
            "weights_hidden_output should change after consolidate",
        )
        self.assertFalse(
            np.array_equal(b_o_before, self.ai.bias_output),
            "bias_output should change after consolidate",
        )

    def test_model_save_load_new_cerebellum_architecture(self):
        # Set known values for all attributes
        self.ai.weights_input_hidden = np.full(
            (self.ai.input_size, self.ai.hidden_size), 0.51
        )
        self.ai.bias_hidden = np.full((1, self.ai.hidden_size), 0.52)
        self.ai.weights_hidden_output = np.full(
            (self.ai.hidden_size, self.ai.output_size), 0.53
        )
        ...
        np.testing.assert_array_almost_equal(
            loaded_ai.weights_hidden_output, self.ai.weights_hidden_output
        )
        np.testing.assert_array_almost_equal(loaded_ai.bias_output, self.ai.bias_output)
    ...
    def test_load_model_backward_compatibility_cerebellum(self):
        ...
        old_model_data = {
            "weights": (
                np.random.rand(self.ai.input_size, self.ai.output_size) * 0.01
            ).tolist()
        }  # Old structure
        ...
        self.assertIsNotNone(
            loaded_ai.weights_input_hidden,
            "weights_input_hidden should be initialized on backward compat load.",
        )
        self.assertEqual(
            loaded_ai.weights_input_hidden.shape,
            (self.ai.input_size, self.ai.hidden_size),
        )
        self.assertIsNotNone(
            loaded_ai.bias_hidden,
            "bias_hidden should be initialized on backward compat load.",
        )
        self.assertEqual(loaded_ai.bias_hidden.shape, (1, self.ai.hidden_size))
        self.assertIsNotNone(
            loaded_ai.weights_hidden_output,
            "weights_hidden_output should be initialized on backward compat load.",
        )
        self.assertEqual(
            loaded_ai.weights_hidden_output.shape,
            (self.ai.hidden_size, self.ai.output_size),
        )
        self.assertIsNotNone(
            loaded_ai.bias_output,
            "bias_output should be initialized on backward compat load.",
        )
    ...
    def test_load_model_shape_mismatch_cerebellum(self):
        ...
        model_data["weights_input_hidden"] = [
            [0.01, 0.02],
            [0.03, 0.04],
        ]  # Example of a clearly wrong shape
        with open(TEST_CEREBELLUM_MODEL_PATH, "w") as f:
            json.dump(model_data, f)

        # Get default weights from a fresh instance
        temp_ai_defaults = CerebellumAI(
            model_path="non_existent_temp_model.json"
        )  # Ensure it initializes defaults
        ...
        loaded_ai = CerebellumAI(
            model_path=TEST_CEREBELLUM_MODEL_PATH
        ...
        np.testing.assert_array_almost_equal(
            ...
            default_w_ih,
            decimal=5,
            err_msg="Corrupted weights_input_hidden not reset to default.",
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.bias_hidden,
            default_b_h,
            decimal=5,
            err_msg="Corrupted bias_hidden not reset to default.",
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.weights_hidden_output,
            default_w_ho,
            decimal=5,
            err_msg="Corrupted weights_hidden_output not reset to default.",
        )
        np.testing.assert_array_almost_equal(
            loaded_ai.bias_output,
            default_b_o,
            decimal=5,
            err_msg="Corrupted bias_output not reset to default.",
        )
...
class TestParietalLobeAI(unittest.TestCase):
    def setUp(self):
        ...
        self.sample_sensory_data = np.random.rand(
            self.ai.input_size
        ).tolist()  # input_size is 20
        self.sample_true_coords = np.random.rand(
            self.ai.output_size
        ).tolist()  # output_size is 3
    ...
    def test_forward_propagate_output_shapes_parietal(self):
        input_vec_1d, hidden_layer_output, output_coords_final = (
            self.ai._forward_propagate(self.sample_sensory_data)
        )
        self.assertEqual(
            input_vec_1d.shape, (self.ai.input_size,), "Input vector shape mismatch."
        )
        ...
        self.assertEqual(
            output_coords_final.shape,
            (self.ai.output_size,),
            "Output coordinates shape mismatch.",
        )
        self.assertTrue(
            np.all(hidden_layer_output >= -1) and np.all(hidden_layer_output <= 1),
            "Hidden output out of Tanh range.",
    ...
    def test_learn_updates_and_memory_parietal(self):
        ...
        expected_mem_sensor = (
            self.sample_sensory_data
            if isinstance(self.sample_sensory_data, list)
            else self.sample_sensory_data.tolist()
        )
        expected_mem_coords = (
            self.sample_true_coords
            if isinstance(self.sample_true_coords, list)
            else self.sample_true_coords.tolist()
        )
        ...
        for i in range(self.ai.max_memory_size + 5):  # Test memory limit
            self.ai.learn(
                self.sample_sensory_data,
                (
                    np.array(self.sample_true_coords)
                    * (i / (self.ai.max_memory_size + 4.0))
                ).tolist(),
            )
    ...
    def test_consolidate_updates_weights_parietal(self):
        self.ai.learn(
            self.sample_sensory_data, self.sample_true_coords
        )  # Populate memory
        initial_params = {
            "w_ih": self.ai.weights_input_hidden.copy(),
            "b_h": self.ai.bias_hidden.copy(),
    ...
    def test_model_save_load_new_parietal_architecture(self):
        # Set known values for all attributes
        self.ai.weights_input_hidden = np.full(
            (self.ai.input_size, self.ai.hidden_size), 0.81
        )
        self.ai.bias_hidden = np.full((1, self.ai.hidden_size), 0.82)
        self.ai.weights_hidden_output = np.full(
            (self.ai.hidden_size, self.ai.output_size), 0.83
        )
    ...
    def test_load_model_backward_compatibility_parietal(self):
        ...
        self.assertIsNotNone(
            loaded_ai.weights_input_hidden,
            "weights_input_hidden should be initialized.",
        )
        self.assertEqual(
            loaded_ai.weights_input_hidden.shape,
            (self.ai.input_size, self.ai.hidden_size),
        ...
        self.assertIsNotNone(
            loaded_ai.bias_hidden, "bias_hidden should be initialized."
        )
        self.assertEqual(loaded_ai.bias_hidden.shape, (1, self.ai.hidden_size))
        self.assertIsNotNone(
            loaded_ai.weights_hidden_output,
            "weights_hidden_output should be initialized.",
        )
        self.assertEqual(
            loaded_ai.weights_hidden_output.shape,
            (self.ai.hidden_size, self.ai.output_size),
        ...
        self.assertIsNotNone(
            loaded_ai.bias_output, "bias_output should be initialized."
        )
        self.assertEqual(loaded_ai.bias_output.shape, (1, self.ai.output_size))
    ...
    def test_load_model_shape_mismatch_parietal(self):
        ...
        loaded_ai = ParietalLobeAI(
            model_path=TEST_PARIETAL_MODEL_PATH
