...
if __name__ == "__main__":
    ...
    if not (len(embedding) == ai.output_size):
        raise AssertionError("Embedding length does not match output_size.")
    ...
    if not (len(embedding) == ai.output_size):
        raise AssertionError("Embedding length from tuple does not match output_size.")
    if not isinstance(visual_label, int):
        raise AssertionError("Visual label is not an integer.")
    ...
    if not (len(ai_loaded.memory_db) == len(ai.memory_db)):
        raise AssertionError("Loaded memory_db size does not match original.")
    if not (len(ai_loaded.cross_modal_memory) == len(ai.cross_modal_memory)):
        raise AssertionError("Loaded cross_modal_memory size does not match original.")
