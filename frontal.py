...
from tensorflow.keras.optimizers.legacy import Adam as LegacyAdam # For M1/M2 Mac compatibility if needed
import random
from collections import deque # For replay buffer

class FrontalLobeAI:
    def __init__(self, input_size=18, output_size=5, model_path="data/frontal_model.weights.h5"):
        ...
        self.epsilon_decay_rate = 0.001 # Make this smaller for longer exploration, e.g., 0.0001 or 0.00001
    ...
    def _build_model(self):
        model = Sequential([
            Dense(32, input_dim=self.input_size, activation='relu', name="dense_input"),
            Dense(32, activation='relu', name="dense_hidden_1"),
            Dense(self.output_size, activation='linear', name="dense_output") # Q-values for each action
        ])
    ...
    def process_task(self, current_input_data): # Renamed from input_data to avoid confusion
    ...
    def learn(self, state, action, reward, next_state, done):
        ...
        if len(self.memory) >= self.replay_batch_size: # Start replay only when enough samples
    ...
    def replay(self):
        ...
        for state_raw, action, reward, next_state_raw, done in minibatch:
            ...
            if not done:
                ...
                next_state_batch_for_predict = np.reshape(next_state_prepared, [1, self.input_size])
                # Use target_model for predicting next Q-values (stabilizes learning)
                target = reward + self.discount_factor_gamma * \
                           np.amax(self.target_model.predict(next_state_batch_for_predict, verbose=0)[0])
            
            # Get current Q-values for the original state from the main model
            current_q_values_for_state = self.model.predict(state_batch_for_predict, verbose=0)
            current_q_values_for_state[0][action] = target # Update Q-value for the action taken
            
            states_prepared.append(state_prepared)
            q_values_updated.append(current_q_values_for_state[0]) # Append the full array of Q-values for this state

        # Train the main model on the batch of experiences
        # This is more efficient than training one by one in the loop
        try:
            self.model.fit(np.array(states_prepared), np.array(q_values_updated), epochs=1, verbose=0)
    ...
    def save_model(self):
        print(f"Saving Frontal Lobe model to {self.model_path} and epsilon to {self.epsilon_path}")
    ...
    def load_model(self):
        if os.path.exists(self.model_path):
            ...
            try:
                ...
                print(f"Error loading Frontal Lobe model weights: {e}. Model remains initialized.")
        else:
            print(f"No pre-trained weights found for Frontal Lobe at {self.model_path}. Model is newly initialized.")
        ...
        if os.path.exists(self.epsilon_path):
            ...
            try:
                with open(self.epsilon_path, 'r') as f:
                    ...
                    self.exploration_rate_epsilon = epsilon_data.get('exploration_rate_epsilon', 1.0)
                print(f"Frontal Lobe epsilon loaded: {self.exploration_rate_epsilon:.3f}")
            except Exception as e:
                print(f"Error loading Frontal Lobe epsilon: {e}. Using default epsilon.")
        else:
            print(f"No epsilon file found at {self.epsilon_path}. Using default epsilon.")
    ...
    def consolidate(self):
        ...
        if len(self.memory) < self.replay_batch_size:
            print("Frontal Lobe: Not enough experiences in memory to consolidate with extensive replay.")
        else:
            # Perform more replay steps for consolidation
            consolidation_replay_count = 10 # Example: 10 replay batches
            print(f"Frontal Lobe: Performing {consolidation_replay_count} replay batches for consolidation.")
        ...
        self.save_model()
        print("Frontal Lobe: Consolidation complete (replayed experiences and saved model).")
...
if __name__ == "__main__":
    ...
    if os.path.exists(test_model_path + "_epsilon.json"): os.remove(test_model_path + "_epsilon.json")
    ...
    for i in range(num_test_steps):
        ...
        if (i+1) % (frontal_ai.replay_batch_size // 2) == 0: # Log progress
             print(f"Step {i+1}: Epsilon: {frontal_ai.exploration_rate_epsilon:.3f}, Memory size: {len(frontal_ai.memory)}")
    ...
    print(f"Epsilon before load: {epsilon_before_load:.3f}, Epsilon after load: {frontal_ai_loaded.exploration_rate_epsilon:.3f}")
    ...
    if abs(frontal_ai_loaded.exploration_rate_epsilon - epsilon_before_load) < 1e-5 : # Allow small tolerance
         print("Epsilon loaded correctly.")
    else:
         print("Epsilon loading discrepancy. Check save/load logic or decay interaction.")


    # A simple check: compare a bias term from the output layer of the main model
    if hasattr(frontal_ai.model.get_layer("dense_output"), "bias") and \
       hasattr(frontal_ai_loaded.model.get_layer("dense_output"), "bias"):
        ...
        if np.array_equal(original_bias, loaded_bias):
            ...
            print("Model weights loaded, but output layer bias term does not match. Investigate.")
    else:
